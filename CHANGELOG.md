...
### Fixed
- 🐛 修复设置窗口重复弹出问题
  - 创建 PreferencesWindowManager 单例类统一管理设置窗口
  - 确保主界面和菜单栏使用同一个设置窗口实例
  - 修复了可以同时打开多个设置窗口的问题
  - 窗口关闭后保持实例以便重用，提升打开速度
...
### Added
- ✨ **标签页管理新功能**
  - 重新设计标签页设置UI，集成功能开关、顺序编辑和默认设置
  - 支持自定义标签页顺序，拖拽排序更直观
  - 新增标签页功能独立开关控制
  - 优化标签页切换功能和悬停动画效果

- 📋 **剪贴板功能增强**
  - 优化粘贴板筛选器，保持展开状态以提升用户体验
  - 添加启动默认筛选面板功能，快速访问常用筛选
  - 优化过滤工具栏与面板交互，改为纯图标设计
  - 类型/日期筛选改为单行横向芯片布局，选择后自动收起
  - 来源筛选仅显示应用图标，界面更简洁

### Changed
- 🎨 **界面优化**
  - 偏好设置改为"侧边栏分类 + 右侧详情"布局
  - 统一界面元素尺寸和间距，增加悬停高亮与选中指示
  - 面板采用互斥展开设计，避免界面混乱
...
### Enhanced
- 🖥️ **窗口管理优化**
  - 改进浮动窗口的键盘输入支持，增强KeyboardSupportPanel功能
  - 优化窗口焦点管理，确保编辑状态下的正确响应
  - 增强自动隐藏逻辑，改进失去焦点时的行为
  - 完善模态窗口支持，提升多窗口环境下的稳定性

- 🖱️ **鼠标和触摸板体验改进**
  - 新增鼠标滚轮模式支持，用户可选择传统或自然滚动方向
  - 优化触摸板双指滑动检测，提升手势识别准确性
  - 改进多屏幕环境下的鼠标位置检测
  - 减少调试日志输出频率，降低系统延迟

### Fixed
- 🐛 修复窗口在某些情况下无法正确获得键盘焦点的问题
- 🔧 修复编辑状态下自动隐藏可能被阻止的问题
- 📱 修复触摸板手势在边缘区域的检测精度问题

### Technical
- 重构KeyboardSupportPanel类，增强键盘事件处理
- 优化EdgeMouseTracker的事件监听机制
- 改进窗口生命周期管理和内存使用
...
### Added
- ⚙️ **配置管理系统重构**
  - 新增 ConfigurationManager 统一管理所有应用配置
  - 实现配置的集中式存储和访问
  - 支持配置项的实时更新和持久化
